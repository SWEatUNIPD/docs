...
#show: content=>verbale(
  ...
  versioni: (
    ...
    "Klaudio Merja\nDavide Picello",
    [
      - Identificato termini del glossario
    ],
...
- *Simulazione GPS*: generazione sensata di dati che possano simulare un percorso di un utente (ad esempio un nuovo dato della posizione ogni 5 secondi). È stato suggerito l'uso di file #text(weight: "bold", underline(".gpx"))#rifGlossario che facilitano il compito.
...
- #underline("stream processor")#rifGlossario che analizza i dati dell'utente
- un punto di interesse
- dashboard per visualizzare gli annunci (utilizzando #underline("Grafana")#rifGlossario o #underline("Apache Superset")#rifGlossario)
...
Riguardo la parte logistica hanno espresso il desiderio di mantenere per tutto il corso del progetto un #underline("SAL")#rifGlossario a settimana in remoto e un ultimo conclusivo a fine progetto in presenza. Mettono a disposizione anche delle sessioni di "deep dive" con le quali si intende aiutare il gruppo nei casi in cui riscontri difficoltà a superare certe attività o a trovare sufficiente documentazione riguardo le tecnologie adottate.
