...
== Dimostrazione MVP
Il gruppo ha esposto il grado di avanzamento del MVP fornendo anche una piccola dimostrazione. Questa consisteva di una simulazione di una decina di utenti visibili sulla _#rifGlossario("dashboard")_ di #rifGlossario("Grafana"). Questo è stato possibile in seguito a delle modifiche proprio all'immagine di Grafana, in quanto nativamente non permetteva di visualizzare più di un percorso alla volta.
...
Infine è stato affrontato il tema della chiamata #rifGlossario("API") usata per condividere l'identificativo tra _client_ e _server_. La proponente ha fatto notare che sarebbe una funzionalità demandata alla applicazione _mobile_ dell'utente, prodotto che non è stato chiesto di sviluppare e quindi non pertinente col capitolato. La soluzione concordata per il MVP è stata quella di selezionare a priori un numero fisso di noleggi che andranno attivati, avendo quindi già controllo degli identificativi. 
