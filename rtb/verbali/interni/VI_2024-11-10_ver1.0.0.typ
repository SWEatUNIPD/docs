...
Dopo alcune proposte stilistiche riguardo al sito dove pubblicare la documentazione abbiamo deciso di aggiornarci alla prossima riunione. Oltre a sistemare l'aspetto del sito bisogna aggiungere la logica che permette di collegare i link del sito ai documenti in pdf. Abbiamo quindi creato la repository *sweatunipd.github.io* che conterrà i file Python che aggiorneranno i link del sito in modo che aprano i documenti desiderati.

== Backlog
Abbiamo deciso di utilizzare il sistema di issue di GitHub come backlog. Poiché abbiamo pianificato di creare più repository a seconda dell'area di azione, ad esempio una per la documentazione e una per il sito che visualizzi i documenti, abbiamo convenuto a creare ua sola *board* che fungesse da backlog. In base alle attività vengono create le issue nelle rispettive repository, così da mantenere la separazione strutturale e poterle collegare alle "commit", ma vengono inserite nella stessa board così da essere tracciabili nello stesso spazio. Per mantenere questa metodologia abbiamo adottato le seguenti regole:
...
== Glossario e README
Per evitare malinterpretazioni tra i termini abbiamo aggiunto un glossario da popolare col proseguimento del progetto. Abbiamo inoltre aggiunto le informazioni generali alle repository nei rispettivi file standard README.md.

= Conclusioni
Come molte riunioni nel primo periodo del progetto questa è servita a modellare nuovamente il Way of Working del gruppo oltre a incrementare la struttura dell'organizzazione. Ci impegnamo quindi a rispettare i punti definiti precedentemente in modalità asincrona e a incontrarci il prima possibile con l'azienda proponente. In caso non fosse possibile pianificheremo una riunione interna.
...
#backlog(
  content: (
    ...
    "Klaudio Merja\nDavide Marin", "Redazione glossario",
  )
)