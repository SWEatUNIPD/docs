...
L'incontro aveva l'obiettivo di fare il punto della situazione a metà _#rifGlossario("sprint")_, valutare l'implementazione delle tecnologie individuate, definire i prossimi _step_ e risolvere eventuali dubbi.
...
_Focus_ particolare è stato dato all'utilizzo di Grafana per visualizzare dati #rifGlossario("GPS") e annunci pubblicitari e all'integrazione logica all'interno del sistema tramite Apache NiFi.
...
È possibile utilizzarli in combinazione. PostGIS sarà utile per gestire dati spaziali e Time Scale per ottimizzare la gestione di dati temporali, mantenendo una visione relazionale classica con un _database_ aggiuntivo.
...
- _#rifGlossario("Mockup")_ dei dati su Grafana.
...
#backlog(content: (
  "ORG",
  "Gruppo",
  "SAL fissato per mercoledì 27/11",
))