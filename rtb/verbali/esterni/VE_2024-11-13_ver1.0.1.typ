...
Ci è stata data autonomia nella scelta di metodologie e strumenti, con l'opzione di chiedere assistenza o riferimenti all'azienda su _best practice_ per versionamento, _commit_ (ad esempio Conventional Commits), #rifGlossario("Git") e GitLab. Per facilitare la collaborazione, si consiglia di mantenere e catalogare le _best practice_ all'interno della _repository_, in cui Andrea Dorigo sarà invitato come collaboratore per monitorare l'avanzamento. È stato suggerito di utilizzare #rifGlossario("Docker Compose") per creare un ambiente containerizzato, evitando l'_overhead_ di #rifGlossario("Kubernetes"), che risulta eccessivo per le necessità attuali.
== Suggerimento milestones
Ci è stato consigliato di definire _milestone_ per documentazione, Proof of Concept (#rifGlossario("PoC")), Minimum Viable Product (#rifGlossario("MVP")) e per il prodotto finale, evitando una frammentazione eccessiva. Ogni _milestone_ rappresenterà un traguardo significativo. A livello di _sprint_, si definiranno obiettivi chiari (non chiamati "_milestone_") per mantenere il progetto costantemente allineato agli obiettivi principali.
